...
spawn(function() 
    ...
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Thì Light X Hub đã trở lại!!!!!<Color=/>"):Display()
    wait(.14)
    ...
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Channel : Sơn Vjp Pờ Gồ :))) <Color=/>"):Display()
    wait(.36)
end)
...
W:AddLabel("Script By Sơn Vjp Pờ Gồ")

local Section = W:AddSection({
    Name = "Update"
})

W:AddLabel("[+] Light X Hub is back!")
